# q₀ 改进方案

**问题**：q₀ 平均敏感概率 0.82 过高，可能导致 Precision 低，在 EMG 融合时引入噪声

---

## 📊 已实施的改进

### 参数调整（已实施）

1. **降低 max_sensitive_prob**：0.95 → 0.75
2. **提高 min_matches_for_sensitive**：1 → 2
3. **降低类别权重**：politics_weight 0.8→0.6, abuse_weight 0.6→0.5
4. **降低缩放因子**：tanh 缩放因子 10 → 5

### 改进效果

- ✅ **NLL**: 从增加 5.70% → 降低 2.72%（改善 8.42 个百分点）
- ✅ **ECE**: 从增加 39.84% → 降低 11.16%（改善 51 个百分点）
- ⚠️ **F1**: 从下降 0.86% → 下降 1.11%（略有恶化）

---

## 🔧 后续改进方向（可选）

### P1: 一致性门控（已实施）
- 仅当 `argmax(p) == argmax(q₀)` 时允许 α(u) 生效
- 否则强制 α=1.0（完全信任模型），避免 q₀ 带偏导致的 F1 损失

### P1: 知识阈值门控（推荐）
- **改进**：阈值应该在验证集上搜索学习，而不是硬编码（如0.6）
- **实现**：类似Day7的α*搜索，在dev集上搜索最优阈值
- **规则**：当 `max(q₀) < threshold` 时设置 α=1.0（完全信任模型）
- **方案**：参见 `知识阈值门控设计方案.md`

### P2: 进一步优化 q₀ 质量
- 词表清洗和审核（使用 `scripts/eval_q0.py` 评估）
- 更精细的概率计算策略

def evaluate_q0(q0_file: str, baseline_file: str = None):
    """评估 q₀ 质量"""
    q0_dict, true_labels, q0_labels = load_q0_and_labels(q0_file, baseline_file)
    
    if len(true_labels) == 0:
        print("⚠️ 无法加载真实标签，请提供 baseline_file")
        return
    
    # 计算指标
    precision = precision_score(true_labels, q0_labels)
    recall = recall_score(true_labels, q0_labels)
    f1 = f1_score(true_labels, q0_labels)
    accuracy = accuracy_score(true_labels, q0_labels)
    
    # 混淆矩阵
    cm = confusion_matrix(true_labels, q0_labels)
    tn, fp, fn, tp = cm.ravel()
    
    print("=" * 60)
    print("q₀ 质量评估")
    print("=" * 60)
    print(f"Precision: {precision:.4f} ({precision*100:.2f}%)")
    print(f"Recall:    {recall:.4f} ({recall*100:.2f}%)")
    print(f"F1:        {f1:.4f} ({f1*100:.2f}%)")
    print(f"Accuracy:  {accuracy:.4f} ({accuracy*100:.2f}%)")
    print()
    print("混淆矩阵:")
    print(f"  TN (真阴性): {tn}")
    print(f"  FP (假阳性): {fp}  ← 误报，需要减少")
    print(f"  FN (假阴性): {fn}")
    print(f"  TP (真阳性): {tp}")
    print()
    
    # 分析平均敏感概率
    avg_p_sensitive = sum(q0[1] for q0 in q0_dict.values()) / len(q0_dict)
    print(f"平均敏感概率: {avg_p_sensitive:.4f}")
    
    # 判断质量
    if precision < 0.70:
        print("⚠️ q₀ Precision < 0.70，质量较差，需要改进")
    elif precision < 0.75:
        print("⚠️ q₀ Precision < 0.75，质量一般，建议改进")
    else:
        print("✅ q₀ Precision ≥ 0.75，质量良好")
    
    return {
        'precision': precision,
        'recall': recall,
        'f1': f1,
        'accuracy': accuracy,
        'avg_p_sensitive': avg_p_sensitive,
        'confusion_matrix': {'tn': int(tn), 'fp': int(fp), 'fn': int(fn), 'tp': int(tp)}
    }

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='评估 q₀ 质量')
    parser.add_argument('--q0-file', type=str, required=True, help='q₀ 文件路径')
    parser.add_argument('--baseline-file', type=str, default=None, help='Baseline 文件路径（包含真实标签）')
    args = parser.parse_args()
    
    evaluate_q0(args.q0_file, args.baseline_file)
```

**使用方法**：
```bash
# 评估 q₀ 质量
python scripts/eval_q0.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl
```

---

## 🔧 改进方案（按优先级）

### 方案1：调整 q₀ 计算参数（最简单，立即生效）

#### 1.1 降低 max_sensitive_prob

**问题**：当前 `max_sensitive_prob = 0.95` 太高，导致匹配到词表时概率过高

**修改 `configs/config.yaml`**：
```yaml
q0:
  max_sensitive_prob: 0.75  # 从 0.95 降低到 0.75
  base_sensitive_prob: 0.1  # 保持不变
```

**或者通过命令行参数**：
```bash
python scripts/q0_builder.py \
    --max-sensitive-prob 0.75 \
    --datasets test
```

**预期效果**：
- 平均敏感概率从 0.82 降低到约 0.65-0.70
- 减少误报，提高 Precision

#### 1.2 提高 min_matches_for_sensitive

**问题**：当前 `min_matches_for_sensitive = 1`，只要匹配到 1 个词就触发敏感

**修改 `configs/config.yaml`**：
```yaml
q0:
  min_matches_for_sensitive: 2  # 从 1 提高到 2，要求至少匹配 2 个词
```

**预期效果**：
- 减少单词误匹配导致的误报
- 提高 Precision，可能略微降低 Recall

#### 1.3 调整类别权重

**问题**：当前权重可能不平衡，导致某些类别过度匹配

**修改 `configs/config.yaml`**：
```yaml
q0:
  porn_weight: 1.0      # 保持不变
  politics_weight: 0.6  # 从 0.8 降低到 0.6（涉政词可能误报多）
  abuse_weight: 0.5    # 从 0.6 降低到 0.5（辱骂词可能误报多）
```

**预期效果**：
- 降低误报率高的类别的权重
- 提高整体 Precision

#### 1.4 调整概率计算缩放因子

**问题**：当前 `tanh(match_score * 10)` 的缩放因子 10 可能太大

**修改 `scripts/q0_builder.py` 第 278 行**：
```python
# 原代码
normalized_score = np.tanh(match_score * 10)  # 缩放因子 10

# 改为更保守的缩放
normalized_score = np.tanh(match_score * 5)  # 降低到 5，使概率增长更平缓
```

**预期效果**：
- 概率增长更平缓，避免过度敏感
- 平均敏感概率降低

---

### 方案2：清洗词表（最有效，但需要时间）

#### 2.1 使用 LLM 清洗词表

**已有功能**，直接使用：

```bash
# 清洗所有类别的词表
python scripts/lexicon_generator.py \
    --clean-lexicon \
    --lexicon-dir configs/lexicons \
    --categories porn politics abuse \
    --clean-batch-size 100 \
    --backup-original

# 清洗单个类别（例如，先清洗误报最多的类别）
python scripts/lexicon_generator.py \
    --clean-lexicon \
    --lexicon-dir configs/lexicons \
    --categories politics \
    --clean-batch-size 100
```

**预期效果**：
- 删除 70-85% 的噪声词
- 显著提高 Precision
- 词表大小减少，匹配效率提升 3-5 倍

#### 2.2 手动检查高频误报词

**步骤**：
1. 运行 `eval_q0.py` 找出误报样本
2. 分析这些样本匹配到的词
3. 手动删除明显误报的词

**创建脚本：`scripts/analyze_q0_errors.py`**

```python
"""
分析 q₀ 误报样本，找出高频误报词
"""
import json
from collections import Counter

def analyze_q0_errors(q0_file: str, baseline_file: str):
    """分析 q₀ 误报"""
    # 加载数据
    q0_dict = {}
    baseline_dict = {}
    
    with open(q0_file, 'r', encoding='utf-8') as f:
        for line in f:
            item = json.loads(line)
            q0_dict[item['id']] = item
    
    with open(baseline_file, 'r', encoding='utf-8') as f:
        for line in f:
            item = json.loads(line)
            baseline_dict[item['id']] = item
    
    # 找出误报（q₀ 预测敏感，但实际非敏感）
    false_positives = []
    false_positive_words = Counter()
    
    for item_id, q0_item in q0_dict.items():
        if item_id not in baseline_dict:
            continue
        
        q0_label = 1 if q0_item['q0'][1] > 0.5 else 0
        true_label = baseline_dict[item_id]['coarse_label']
        
        if q0_label == 1 and true_label == 0:  # 误报
            false_positives.append({
                'id': item_id,
                'text': q0_item.get('text', ''),
                'q0': q0_item['q0'],
                'matched_words': []
            })
            
            # 收集匹配到的词
            if 'q0_details' in q0_item:
                details = q0_item['q0_details']
                for word in details.get('porn_matches', []):
                    false_positive_words[word] += 1
                for word in details.get('politics_matches', []):
                    false_positive_words[word] += 1
                for word in details.get('abuse_matches', []):
                    false_positive_words[word] += 1
    
    # 输出结果
    print(f"误报样本数: {len(false_positives)}")
    print(f"\n高频误报词（Top 20）:")
    for word, count in false_positive_words.most_common(20):
        print(f"  {word}: {count} 次")
    
    # 保存误报样本
    with open('output/q0_false_positives.jsonl', 'w', encoding='utf-8') as f:
        for item in false_positives[:100]:  # 保存前100个
            f.write(json.dumps(item, ensure_ascii=False) + '\n')
    
    print(f"\n误报样本已保存到: output/q0_false_positives.jsonl")

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--q0-file', type=str, required=True)
    parser.add_argument('--baseline-file', type=str, required=True)
    args = parser.parse_args()
    
    analyze_q0_errors(args.q0_file, args.baseline_file)
```

**使用方法**：
```bash
python scripts/analyze_q0_errors.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl
```

---

### 方案3：改进概率计算逻辑（更保守的策略）

#### 3.1 使用更保守的概率映射

**修改 `scripts/q0_builder.py` 的概率计算部分**：

```python
# 原代码（第 278-279 行）
normalized_score = np.tanh(match_score * 10)
p_sensitive = base_sensitive_prob + (max_sensitive_prob - base_sensitive_prob) * (normalized_score + 1) / 2

# 改为更保守的策略
def compute_conservative_prob(match_score, total_matches, text_words, 
                              base_sensitive_prob=0.1, max_sensitive_prob=0.75):
    """
    更保守的概率计算：
    1. 要求匹配密度足够高（匹配词数 / 文本长度）
    2. 使用更平缓的概率增长曲线
    """
    # 计算匹配密度
    match_density = total_matches / max(text_words, 1)
    
    # 如果匹配密度太低，使用基础概率
    if match_density < 0.1:  # 匹配词占比 < 10%
        return base_sensitive_prob
    
    # 使用 sigmoid 函数，更平缓
    # sigmoid(x) = 1 / (1 + exp(-k*x))
    # 调整 k 使概率增长更平缓
    k = 3.0  # 从原来的 10 降低到 3
    sigmoid_score = 1 / (1 + np.exp(-k * match_score))
    
    # 映射到 [base_sensitive_prob, max_sensitive_prob]
    p_sensitive = base_sensitive_prob + (max_sensitive_prob - base_sensitive_prob) * sigmoid_score
    
    return p_sensitive
```

#### 3.2 基于匹配数量的阈值策略

**修改 `scripts/q0_builder.py`**：

```python
# 在 compute_q0 函数中，修改概率计算逻辑
if total_matches < min_matches_for_sensitive:
    p_sensitive = base_sensitive_prob
elif total_matches == 1:
    # 只匹配到 1 个词，使用较低概率
    p_sensitive = base_sensitive_prob + (max_sensitive_prob - base_sensitive_prob) * 0.3
elif total_matches == 2:
    # 匹配到 2 个词，使用中等概率
    p_sensitive = base_sensitive_prob + (max_sensitive_prob - base_sensitive_prob) * 0.6
else:
    # 匹配到 3 个或更多词，使用较高概率
    normalized_score = np.tanh(match_score * 5)  # 降低缩放因子
    p_sensitive = base_sensitive_prob + (max_sensitive_prob - base_sensitive_prob) * (normalized_score + 1) / 2
```

---

### 方案4：改进匹配策略

#### 4.1 要求多个类别同时匹配

**修改 `scripts/q0_builder.py`**：

```python
# 在 compute_q0 函数中，添加多类别匹配要求
porn_matches = lexicon_matches.get('porn', [])
politics_matches = lexicon_matches.get('politics', [])
abuse_matches = lexicon_matches.get('abuse', [])

# 要求至少 2 个类别都有匹配，或者单个类别匹配数 >= 3
categories_with_matches = sum([
    len(porn_matches) > 0,
    len(politics_matches) > 0,
    len(abuse_matches) > 0
])

if categories_with_matches < 2 and total_matches < 3:
    # 不满足条件，使用基础概率
    p_sensitive = base_sensitive_prob
else:
    # 正常计算
    ...
```

#### 4.2 使用词边界匹配（避免子串匹配）

**修改 `scripts/q0_builder.py` 的 `match_lexicon` 函数**：

```python
import re

def match_lexicon_with_word_boundary(text: str, lexicons: Dict[str, Set[str]]):
    """
    使用词边界匹配，避免子串误匹配
    例如："连衣裙" 不会匹配到 "裙"
    """
    matches = {category: [] for category in lexicons.keys()}
    
    for category, words in lexicons.items():
        for word in words:
            # 使用词边界正则
            pattern = r'\b' + re.escape(word) + r'\b'
            if re.search(pattern, text, re.IGNORECASE):
                matches[category].append(word)
    
    return matches
```

---

## 🎯 推荐改进流程

### 第一步：评估当前 q₀ 质量

```bash
# 1. 创建评估脚本（如果还没有）
# 2. 运行评估
python scripts/eval_q0.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl
```

### 第二步：快速调整参数（立即生效）

```bash
# 修改 config.yaml 或使用命令行参数
python scripts/q0_builder.py \
    --max-sensitive-prob 0.75 \
    --min-matches-for-sensitive 2 \
    --politics-weight 0.6 \
    --abuse-weight 0.5 \
    --datasets test

# 重新评估
python scripts/eval_q0.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl
```

### 第三步：清洗词表（如果参数调整不够）

```bash
# 清洗词表
python scripts/lexicon_generator.py \
    --clean-lexicon \
    --lexicon-dir configs/lexicons \
    --categories porn politics abuse \
    --backup-original

# 重新生成 q₀
python scripts/q0_builder.py --datasets test

# 重新评估
python scripts/eval_q0.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl
```

### 第四步：分析误报并手动优化

```bash
# 分析误报
python scripts/analyze_q0_errors.py \
    --q0-file data/q0_test.jsonl \
    --baseline-file output/test_with_uncertainty.jsonl

# 查看高频误报词，手动删除
# 然后重新生成 q₀ 和评估
```

### 第五步：重新运行 Day7-Day9

```bash
# 重新运行 Day7（使用新的 q₀）
python scripts/emg_bucket_search.py

# 重新运行 Day8
python scripts/emg_fit_alpha_u.py

# 重新运行 Day9
python scripts/eval_emg.py
```

---

## 📊 预期改进效果

### 目标指标

- **q₀ Precision**: 从 < 0.70 提升到 ≥ 0.75
- **q₀ F1**: 从 < 0.75 提升到 ≥ 0.80
- **平均敏感概率**: 从 0.82 降低到 0.65-0.70
- **EMG F1**: 从 88.36% 提升到 ≥ 89.13%（超过 baseline）

### 改进优先级

1. **🥇 调整参数**（最简单，立即生效）
   - 降低 `max_sensitive_prob` 到 0.75
   - 提高 `min_matches_for_sensitive` 到 2
   - 调整类别权重

2. **🥈 清洗词表**（最有效，但需要时间）
   - 使用 LLM 清洗，删除 70-85% 噪声词

3. **🥉 改进概率计算**（需要修改代码）
   - 更保守的概率映射
   - 基于匹配数量的阈值策略

4. **改进匹配策略**（需要修改代码）
   - 要求多类别匹配
   - 词边界匹配

---

**建议**：先尝试方案1（调整参数），如果不够再尝试方案2（清洗词表），最后考虑方案3和4（修改代码）。


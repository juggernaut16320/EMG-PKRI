...
## 高不确定性切片验证结果（2025-12-14）

### 切片评估结果

**按不确定性阈值分段评估**：
- **低不确定性**：u < 0.1（3913 个样本，79%）
- **中等不确定性**：0.1 ≤ u < 0.3（653 个样本，13%）
- **高不确定性**：u ≥ 0.3（382 个样本，8%）
...
1. ✅ **低不确定性切片表现优秀**：
   - EMG 完美保持性能（F1 不变）
   - 同时显著改善概率质量（NLL 降低 11.8%）
   - 符合理论预期（低 u 时 α 大，更信任模型）
...
#### 需要改进的部分
1. ❌ **高不确定性切片存在严重问题**：
   - 虽然 F1 略升（+1.14%），但 NLL 和 ECE 大幅恶化
   - 根本原因：q₀ 质量差，完全依赖（α=0.00）导致问题放大
...
#### 改进方向
1. **立即实施一致性门控**（P1）：
   - 仅当 `argmax(p) == argmax(q₀)` 时允许融合
   - 可以防止 q₀ 把模型带偏

2. **立即实施知识阈值门控**（P1）：
   - 当 `max(q₀) < 0.6` 时强制 α=1.0（完全信任模型）
   - 避免使用弱知识

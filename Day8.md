...
**1. 查表文件：** `output/alpha_u_lut.json`
**2. 曲线图：** `output/alpha_u_curve.png` (99.1 KB)
...
```json
{
  "u": [0.0156, 0.0200, 0.0244, 0.0288, 0.0332, ...],
  "alpha": [0.7500, 0.7374, 0.7247, 0.7121, 0.6994, ...]
}
```
...
**图表内容：**
- ✓ 原始 α* 数据点（5个 bucket，蓝色散点）
- ✓ PAV 拟合的单调递减曲线（红色线）
- ✓ 单调性验证：通过（alpha 随 u 单调递减）

**结果解读：**
- ✓ 低不确定性（u < 0.02）：alpha ≈ 0.75，更信任模型预测
- ✓ 中等不确定性（0.1 < u < 0.3）：alpha 逐渐降低，平衡模型和知识
- ✓ 高不确定性（u > 0.43）：alpha = 0.00，完全依赖知识后验 q₀
- ✓ 符合 EMG 理论预期：不确定性越高，越信任知识
...
**实际运行结果：**
- ✅ 成功生成 `output/alpha_u_lut.json`（100个查表点）
- ✅ 成功生成 `output/alpha_u_curve.png`（99.1 KB）
- ✅ 单调递减性验证通过
- ✅ u范围 [0.0156, 0.4510]，alpha范围 [0.0000, 0.7500]
